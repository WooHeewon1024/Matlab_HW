...
b = mean(x_temp,'omitnan');
x_clean = x_temp;
x_clean(isnan(x_clean)) = b;
...
idxA = strcmp(Material,'A');
idxB = strcmp(Material,'B');
idxC = strcmp(Material,'C');
meanA = mean(EnergyDensity(idxA));
meanB = mean(EnergyDensity(idxB));
meanC = mean(EnergyDensity(idxC));
stdA = std(EnergyDensity(idxA));
stdB = std(EnergyDensity(idxB));
stdC = std(EnergyDensity(idxC));
...
a = mean(T.Age,'omitnan');
T.Age(ismissing(T.Age)) = a;
muFare = mean(T.Fare);
sigFare = std(T.Fare);
...
a = mean(signal,'omitnan');
signalFilled = signal;
signalFilled(idxMissing) = a;
b = mean(signalFilled);
c = std(signalFilled);
idxOutlier = abs(signalFilled - b) > 3 * c;
...
hold on
Z0 = zeros(size(Z));
surf(X,Y,Z0,'FaceAlpha',0.3,'EdgeColor','none')
maxZ = max(Z(:));
minZ = min(Z(:));
[a,b] = find(Z == maxZ,1);
[c,d] = find(Z == minZ,1);
...
plot3(maxPoint(1),maxPoint(2),maxPoint(3),'o','MarkerSize',10,'LineWidth',2)
plot3(minPoint(1),minPoint(2),minPoint(3),'s','MarkerSize',10,'LineWidth',2)
